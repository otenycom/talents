...
| **odoo_client** | The model tool for Odoo `/json/2/` calls. Every call passes `connection=<name>` to select which odoo bind to use (URL, database, API key come from env). |
| **channel** | Where the bot **talks** — the team chat surface. `routing.channel: telegram` → private Telegram DM; `routing.channel: discuss` → Odoo Discuss channel (business bots). |
| **home_connection** | Under `routing:` — the **odoo** connection name Discuss polling uses (`OTENY_HOME_CONNECTION` → that bind's `OTENY_CONN_<NAME>_*` vars). Set this on every Discuss business bot. |

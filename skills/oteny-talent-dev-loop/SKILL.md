...
| Traces | `oteny traces --ref <clone> [--session <id>] [--photos]` | The structured session/turn/message debug trace — the agent's debugging eye. The window is sized to one filing walk: the newest 300 message rows per session and the newest 300 browser rows, rendered oldest first; a tool result keeps 800 characters, other rows 160. A browser result that carries the platform's own keys (`picked`, `resolved_by`, `options`, `source` / `platform_reason`, a halt) gets one `[verdict] {…}` line under its preview, because those keys sit at the end of a result that starts with a long tree and the preview never reaches them. The text form adds one `[browser] <kind> target=… ok=… error=…` line per browser action, so a scenario marker or `absent` can name a browser fact. Pass `--session <task id>` to window a long walk on its own session. `page_snapshot` browser rows cover EVERY turn of a run, including the turns after a human login hand-off (donated sessions) and every follow-up turn — since 2026-08-25; a run before that date can show gaps there. |
